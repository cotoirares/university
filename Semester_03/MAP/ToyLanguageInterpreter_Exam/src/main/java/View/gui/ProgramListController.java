...
public class ProgramListController {
    ...
    public void initialize() {
        ...
        // Example 12: int a; int b; int c; a=1;b=2;c=5; (switch(a*10)
        ...
        // Example 13: Ref int v1; int cnt;
        //new(v1,1);createSemaphore(cnt,rH(v1)); fork(acquire(cnt);wh(v1,rh(v1)*10);print(rh(v1));release(cnt));
        ...
        new CompStatement(
            ...
            new CompStatement(
                new CreateSemaphoreStatement("cnt", new RefExpression(new VariableExpression("v1"))),
                new CompStatement(
                    new ForkStatement(
                        new CompStatement(
                            ...
                            new CompStatement(
                                new WriteHeapStatement("v1", 
                                    new ArithmeticExpression("*", 
                                        new RefExpression(new VariableExpression("v1")), 
                                        new ValueExpression(new IntValue(10)))),
                                new CompStatement(
                                    new PrintStatement(new RefExpression(new VariableExpression("v1"))),
        ...
        // update list view with program numbers
        ObservableList<String> programStrings = FXCollections.observableArrayList();
        for (int i = 0; i < programs.size(); i++) {
            programStrings.add(String.format("Example %d: %s", i + 1, programs.get(i).toString()));
        }
    ...
    private void executeProgram() {
        ...
        try {
            ProgState prgState = new ProgState(new MyStack<>(), new MyDictionary<>(), new MyList<>(), program, new MyDictionary<>(), new MyHeap<>(), new SemaphoreTable());
            int programNumber = index + 1;
            String logFile = String.format("log%d.txt", programNumber);
